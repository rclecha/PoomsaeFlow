...
- TransientOutcome: user-facing input to SessionController.recordOutcome(_:) — cases .retry,
  .passed, .skipped. Never persisted. SessionController maps .passed → .passedAfterRetry
  internally when retryCount > 0, then writes an AttemptOutcome to SwiftData.
- AttemptOutcome: the persisted value written to FormAttempt — .passed, .passedAfterRetry,
  .skipped. Does not contain .retry. Callers always pass TransientOutcome, never AttemptOutcome.
- UserIdentityProvider / AnonymousIdentityProvider: protocol + concrete type in
  Data/Repositories/. Generates a stable anonymous UUID on first access and stores it in
  UserDefaults (com.ryan.PoomsaeFlow.anonymousUserID). Injected into SessionController via
  init(session:userID:) so tests can assert on FormAttempt.userID without touching UserDefaults.
...
## v1.5 scope (shipped)
- Coverage posts to GitHub Actions job summary on every CI run (xcrun xccov + python3 parsing);
  filter excludes Views, App file, test files, Helpers, ShakeEffect; logic-layer coverage: 81.0%
- 72 new unit tests across 6 new test files + HomeViewModelTests extensions; total 164 tests
  (up from 86). New files: FormFamilyTests, FormsDataSourceTests, FormRepositoryTests,
  PracticeSessionTests, PinnedFormsTests, UserPrefsRepositoryTests
- DefaultUserPrefsRepository gained init(defaults: UserDefaults) for test injection
- TransientOutcome extracted from AttemptOutcome — transient and persistent outcomes are now
  separate types (see Core types above)
- UserIdentityProvider protocol + AnonymousIdentityProvider added; SessionController.userID
  is injected rather than generated inline
- SessionController.isComplete and currentForm delegate to PracticeSession; [safe:] subscript
  removed
- SessionViewModel.userTappedPin routes through PinnedForms.adding/removing; retryCount
  forwarding property added
- PinnedForms.empty static var and removingAll(_:) single-pass batch removal added
- HomeViewModel: formsByID and makeTrainingProfile(belt:) extracted; confirmSchoolSwitch uses
  removingAll
- BeltSystemPreset.spartaFormIDs uses FormsDataSource.FormID constants — compile-time UUID safety
- FormsDataSource.FormID promoted from private to internal
- precondition guard in HomeViewModel.init against custom profile crash
- encoder/decoder promoted to static let in DefaultUserPrefsRepository
- FormRepository.forms(for:) uses self.all; SessionBuilder familyRank promoted to static let
- Bug fixed: Hwarang was incorrectly included in spartaFormIDs via a silently wrong raw UUID;
  now correctly excluded. Sparta TKD catalog: 27 forms (2 Keecho + 8 Taegeuk + 8 Palgwe +
  9 Black Belt)
